...
class DataSourcesControllerTest < ActionDispatch::IntegrationTest
  ...
  test 'should create data_source' do
    assert_difference('DataSource.count') do
      post data_sources_url, params: {
        data_source: { base_url: @data_source.base_url, description: @data_source.description, name: @data_source.name }
      }
    end
  ...
  test 'should update data_source' do
    patch data_source_url(@data_source), params: {
      data_source: { base_url: @data_source.base_url, description: @data_source.description, name: @data_source.name }
    }
    assert_redirected_to edit_data_source_url(@data_source)
  end
